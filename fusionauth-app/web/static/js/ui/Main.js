...
FusionAuth.UI.Main = {
  initialize: function() {
    ...
    document.addEventListener('click', (event) =>{
      ...
      if (target.matches('input[type=checkbox]') && target.closest('label.toggle') !== null) {
      // If this toggle in a form row, ignore pointer events on the entire for-row to avoid making a change due to a label click
      const formRow = target.closest('.form-row');
      const element = formRow !== null ? formRow : target.closest('label.toggle');
    ...
    Prime.Document.query('[data-slide-open]').each(function(e) {
      e.addEventListener('change', function() {
        ...
        if (Prime.Utils.isDefined(dataSet.slideOpenValue)) {
          values.push(dataSet.slideOpenValue);
        } else if (Prime.Utils.isDefined(dataSet.slideOpenValues)) {
          // allow for a comma delimited list of values as well
          values.push(...dataSet.slideOpenValues.split(','));
        }

        if (values.length > 0) {
          if (e.is('[type=checkbox], [type=radio]')) {
            if (values.includes(e.getValue())) {
              if (e.isChecked()) {
