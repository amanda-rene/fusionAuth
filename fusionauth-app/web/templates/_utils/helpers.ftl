  ...
  [#elseif field.control == "text"]
    [#if field.type == "date"]
      [#if placeholder?? && placeholder != ""]
        [#local type = "text"/]
        [#local class = "date-picker" /]
      [#else]
        [#local type = "date"/]
        [#local class = "" /]
      [/#if]
      [@control.text id="${fieldId}" type="${type}" name="${key}" required=field.required autofocus=autofocus class="${class}" data_date_only=true labelValue="${theme.optionalMessage(key)}" placeholder=placeholder tooltip="${tooltip}"/]
    [#else]
...
[#function anyFieldHasErrors fieldList]
