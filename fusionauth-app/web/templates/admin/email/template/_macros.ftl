      ...
      <table id="localization-table" data-template="localization-template">
        ...
        <tbody>
          ...
          <tr data-locale="${locale}" data-from-name="${((emailTemplate.localizedFromNames(locale))!'')}" data-html-template="${((emailTemplate.localizedHtmlTemplates(locale))!'')}" data-subject="${((emailTemplate.localizedSubjects(locale))!'')}" data-text-template="${((emailTemplate.localizedTextTemplates(locale))!'')}">
            ...
            <td class="action">
              [@button.actionMenu]
                [@button.actionMenuItem href="/ajax/email/template/validate-localization" icon="edit" textKey="edit"/]
                [@button.actionMenuItem href="/ajax/email/template/preview" icon="search" textKey="view"/]
                [@button.actionMenuItem href="/ajax/email/template/delete-localization" icon="trash" textKey="delete" class="delete-button"/]
              [/@button.actionMenu]
            </td>
          </tr>
          [/#list]
  ...
  <tbody>
    [#list results![] as result]
      <tr>
        ...
        <td class="action">
          [@button.actionMenu]
            [@button.actionMenuItem href="edit/${result.id}" icon="edit" textKey="edit"/]
            [@button.actionMenuItem href="add?emailTemplateId=${result.id}" icon="copy" textKey="duplicate"/]
            [@button.actionMenuItem href="/ajax/email/template/test/${result.id}" icon="envelope" textKey="send-test"/]
            [@button.actionMenuItem href="/ajax/email/template/view/${result.id}" icon="search" textKey="view" ajaxView=true ajaxWideDialog=true resizeDialog=true/]
            [@button.actionMenuItem href="/ajax/email/template/delete/${result.id}" icon="trash" textKey="delete" ajaxForm=true/]
          [/@button.actionMenu]
        </td>
      </tr>
    [#else]
