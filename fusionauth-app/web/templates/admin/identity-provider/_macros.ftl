  ...
  <table class="hover" id="idp-tenant-config-table" data-template="idp-tenant-config-row-template" data-add-button="idp-tenant-config-add-button" data-default-max-count="${defaultIdentityProviderTenantConfiguration.limitUserLinkCount.maximumLinks}" data-no-results="${message.inline('no-search-results')}">
    ...
    [#list identityProvider.tenantConfiguration as tenantId, tenantConfiguragion]
        <tr data-tenant-id="${tenantId}">
          ...
          <td class="action">
            [@button.iconLinkWithText href="#" color="red" icon="trash" textKey="delete" class="delete-button"/]
          </td>
        </tr>
    [/#list]
    </tbody>
...
<table id="identity-providers-table" class="hover listing" data-sortable="false">
  ...
  <tbody>
    [#list results![] as provider]
      <tr>
        <td class="break-word" style="padding: 2px 8px;">
        ...
        <td class="action">
          [@button.actionMenu]
            [@button.actionMenuItem href="/admin/identity-provider/edit/${provider.type}/${provider.id}" icon="edit" textKey="edit"/]
            [@button.actionMenuItem href="/ajax/identity-provider/view/${provider.id}" icon="search" textKey="view" ajaxView=true ajaxWideDialog=true resizeDialog=true/]
            [@button.actionMenuItem href="/admin/identity-provider/delete/${provider.id}" icon="trash" textKey="delete"/]
          [/@button.actionMenu]
        </td>
      </tr>
    [#else]
