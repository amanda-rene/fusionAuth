    ...
    [@layout.pageHeader titleKey="page-title" includeSave=false includeCancel=false breadcrumbs={"/admin/reactor/": "reactor"}]
      [#if status.licensed]
        [#if status.breachedPasswordDetection == "ACTIVE" || status.breachedPasswordDetection == "DISCONNECTED"]
          [@button.ajaxLink href="/ajax/reactor/regenerate-key?csrfToken=${csrfToken}" id="regenerate-key" color="orange" icon="refresh" tooltipKey="{tooltip}regenerate" additionalClass="square button" ajaxForm=true ajaxWideDialog=true/]
        [/#if]
    ...
    [@layout.main]
      ...
      [#if !status.licensed]
        <div class="row col-xs">
          [@message.alert message.inline("{notice}free-license") "info" "info-circle" false/]
        </div>
        [@panel.full titleKey="activate" panelClass="panel transparent start-reactor"]
          <p>[@message.print key="{description}reactor-activate-1"/]</p>
          <p>
            [@message.print key="{description}reactor-activate-2"/]
            <ul>
              <li>[@message.print key="{description}reactor-activate-2-option-1"/]</li>
              <li>[@message.print key="{description}reactor-activate-2-option-2"/]</li>
              <li>[@message.print key="{description}reactor-activate-2-option-3"/]</li>
            </ul>
            [@message.print key="{description}reactor-activate-3"/]
           </p>
           <div class="col-lg-12 col-md-12 col-sm-12 tight-left mb-4 mt-4">
      ...
      [@panel.full]
        ...
        <fieldset>
          [@properties.table classes="properties mt-4"]
            ...
            [@properties.rowEval nameKey="advancedOAuthScopes" object=status propertyName="advancedOAuthScopes"/]
            ...
            [@properties.rowNestedValue nameKey="webAuthn"]
               [#if status.webAuthn == "UNKNOWN"]
                  [@message.print key="licensed-community-feature"/]
               [#else]
