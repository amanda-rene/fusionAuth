...
[#-- @ftlvariable name="userinfoPopulateLambda" type="io.fusionauth.domain.Lambda" --]
...
[#--    [@properties.row nameKey="simpleLogin" value=(fusionAuthHost + "/app/login/" + application.oauthConfiguration.clientId) /]--]
  ...
  [@properties.table]
    ...
    [#if userinfoPopulateLambda?has_content]
      [@properties.row nameKey="lambdaConfiguration.userinfoPopulateName" value=properties.display(userinfoPopulateLambda, "name") /]
      [@properties.row nameKey="lambdaConfiguration.userinfoPopulateId" value=properties.display(userinfoPopulateLambda, "id") /]
    [#else]
      [@properties.row nameKey="lambdaConfiguration.userinfoPopulateId" value=message.inline("none-selected-lambda-disabled")/]
    [/#if]
  [/@properties.table]

  [#-- OAuth scopes configuration --]
  <h3>[@message.print key="scopes-configuration"/]</h3>
  [@properties.table]
    [@properties.row nameKey="scopesConfiguration.relationship" value=message.inline("OAuthApplicationRelationship.${application.oauthConfiguration.relationship}")/]
    [#if application.oauthConfiguration.relationship == "ThirdParty"]
      [@properties.row nameKey="scopesConfiguration.consentMode" value=message.inline("OAuthScopeConsentMode.${application.oauthConfiguration.consentMode}")/]
    [/#if]
    [@properties.row nameKey="scopesConfiguration.unknownScopePolicy" value=message.inline("UnknownScopePolicy.${application.oauthConfiguration.unknownScopePolicy}")/]
    [@properties.row nameKey="scopesConfiguration.scopeHandlingPolicy" value=message.inline("OAuthScopeHandlingPolicy.${application.oauthConfiguration.scopeHandlingPolicy}")/]
    <tr>
      <td class="top" style="padding-left: 0"> 
        [@message.print key="provided-scopes"/]
      ...
      <td style="padding-left: 0">
        <table class="nested">
          ...
          <tr>
            ...
            <th class="text-center">[@message.print key="scope-enabled"/]</th>
            <th class="text-center">[@message.print key="scope-required"/]</th>
          </tr>
          </thead>
          <tbody>

          [#list ["address", "email", "phone", "profile"] as scope]
            <tr>
              <td><code>${scope}</code></td>
              <td class="text-center">${properties.display(application.oauthConfiguration.providedScopePolicy, scope + ".enabled")}</td>
              <td class="text-center">${properties.display(application.oauthConfiguration.providedScopePolicy, scope + ".required")}</td>
            </tr>
          [/#list]
          </tbody>
        </table>
  ...
  [#-- Scopes --]
  <h3>[@message.print key="scopes"/]</h3>
  <table>
    <thead>
    <tr>
      <th>[@message.print key="name"/]</th>
      <th>[@message.print key="id"/]</th>
      <th class="text-center">[@message.print key="required"/]</th>
      <th>[@message.print key="description"/]</th>
    </tr>
    </thead>
    <tbody>
      [#if (application.scopes)?has_content]
        [#list application.scopes as scope]
        <tr>
          <td>${properties.display(scope, 'name')}</td>
          <td>${properties.display(scope, 'id')}</td>
          <td class="text-center">${properties.display(scope, 'required')}</td>
          <td>${properties.display(scope, 'description')}</td>
        </tr>
        [/#list]
      [#else]
      <tr>
        <td colspan="3">[@message.print key="no-scopes"/]</td>
      </tr>
      [/#if]
    </tbody>
  </table>
