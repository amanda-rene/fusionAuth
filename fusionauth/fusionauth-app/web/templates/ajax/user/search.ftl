...
<table class="hover" data-user-table data-sortable="false">
  ...
  <tbody>
      ...
      <tr>
        ...
        <td class="action">
          [@button.actionMenu]
            [@button.actionMenuItem href="manage/${result.id}?tenantId=${result.tenantId}" icon="address-card-o" textKey="manage"/]
            [#if !readOnly]
              [#if result.active]
                [#if result.id != ftlCurrentUser.id]
                  [@button.actionMenuItem href="/ajax/user/deactivate/${result.id}?tenantId=${result.tenantId}" icon="unlock-alt" textKey="lock" ajaxForm=true/]
                [/#if]
              [#else]
                [@button.actionMenuItem href="/ajax/user/reactivate/${result.id}?tenantId=${result.tenantId}" icon="lock" textKey="unlock" ajaxForm=true/]
              [/#if]
            [/#if]
          [/@button.actionMenu]
        </td>
